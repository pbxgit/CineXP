export default async function handler(request) {
    ...
    if (!tmdbApiKey) {
        return new Response(JSON.stringify({ message: 'Server configuration error: TMDB_API_KEY is missing.' }), { status: 500 });
    }
    ...
    try {
        ...
        return new Response(JSON.stringify({ message: error.message || 'Failed to fetch data from TMDb.' }), { status: 500 });
    }
}
