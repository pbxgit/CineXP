...
=====================================================
    ...
    (UPDATED with Seasons Browser and Dynamic Watch Links)
=====================================================
*/
...
function displayDetails(data, type) {
    ...
    // --- DYNAMIC WATCH URL LOGIC ---
    let watchUrl = '#';
    if (type === 'movie') {
        watchUrl = `https://www.cineby.app/movie/${details.id}?play=true`;
    } else if (type === 'tv' && details.seasons && details.seasons.length > 0) {
        // Default watch button to Season 1, Episode 1
        watchUrl = `https://www.cineby.app/tv/${details.id}/1/1?play=true`;
    }
    ...
    const detailsHTML = `
        <div class="details-content-overlay">
            ...
            <div class="details-buttons" style="display: flex; gap: 1rem; margin-top: 1.5rem;">
                <a href="${watchUrl}" target="_blank" class="btn btn-primary" style="padding: 0.75rem 1.5rem; border: none; border-radius: 6px; background-color: var(--color-accent); color: white; font-weight: bold; cursor: pointer; text-decoration: none;">Watch Now</a>
    ...
    // --- RENDER SEASONS BROWSER IF IT'S A TV SHOW ---
    if (type === 'tv' && details.seasons_details && details.seasons_details.length > 0) {
        renderSeasonsBrowser(details.seasons_details, details.id);
    }
    
    setupWatchlistButton(details, type);
}

function renderSeasonsBrowser(seasons, showId) {
    const container = document.getElementById('details-body-content');
    if (!container) return;
        ...
        <section class="seasons-section">
            <div class="seasons-header">
                ...
                <select id="season-selector">
                    ${validSeasons.map(season => `<option value="${season.season_number - 1}">${season.name}</option>`).join('')}
                </select>
            </div>
    ...
    const displayEpisodes = (seasonIndex) => {
        ...
        grid.innerHTML = selectedSeason.episodes.map(ep => {
                ...
                <div class="episode-card">
                    ...
                    <div class="episode-info">
                        <div class="episode-title-container">
                             ...
                             <a href="${watchUrl}" target="_blank" class="episode-watch-link">Watch</a>
                        </div>
    ...
    // Initial display of the first season
...
// 6. WATCHLIST LOGIC (No changes needed here)
...
// 7. UI COMPONENTS & HELPERS (No changes needed here)
...
// 8. EVENT LISTENERS & OBSERVERS (No changes needed here)
