...
=====================================================
    ...
    (Final Version with Skeletons and UI Overhaul)
=====================================================
...
async function initHomePage() {
    ...
    if (trendingMoviesResponse && trendingMoviesResponse.results) {
        ...
        if (heroSkeleton) heroSkeleton.remove(); // Remove hero skeleton
    ...
    if (shelvesSkeleton) shelvesSkeleton.remove(); // Remove all shelf skeletons

    // Make the real shelves visible
    document.getElementById('top-10-shelf').style.display = 'block';
    document.getElementById('trending-shelf').style.display = 'block';
    document.getElementById('popular-shelf').style.display = 'block';
...
function populateTop10Shelf(items) {
    ...
    if (!shelf) return;
    shelf.innerHTML = `
        <h2 class="shelf-title">Top 10 Movies This Week</h2>
...
function populateShelf(shelfId, items, type) {
    ...
    shelf.innerHTML = `
        ...
        <div class="media-scroller"><div class="media-scroller-inner">
            ${items.map(item => createMediaCard(item, type)).join('')}
        </div></div>
    `;
}
