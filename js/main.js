...
async function openDetailsModal(mediaItem) {
    ...
    DOM.modalBackdrop.style.backgroundImage = '';
    ...
    // FIX: Safely check for episode_run_time array and its length
    const runtime = details.runtime ?
        `${details.runtime} min` :
        (details.episode_run_time && details.episode_run_time.length > 0 ? `${details.episode_run_time[0]} min` : '');
    const rating = details.certification || 'N/A';

    // FIX: Use optional chaining and filter before mapping to prevent errors/broken images
    const castHtml = details.cast?.filter(member => member.profile_path).map(member => `
    ...
    const seasonsHtml = (mediaType === 'tv' && details.seasons?.length > 0) ?
        ...
        <div class="modal-seasons">
            ...
            <div class="seasons-list">
                ${details.seasons.filter(season => season.poster_path).map(season => `
