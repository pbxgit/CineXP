...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        setupEventListeners();
        setupCarouselEnterAnimation(); // [REPLACED] New animation setup
...
function setupEventListeners() {
    ...
    window.addEventListener('mousemove', (e) => { const { clientX, clientY } = e; const { innerWidth, innerHeight } = window; const x = ((clientX / innerWidth) - 0.5) * 2; const y = ((clientY / innerHeight) - 0.5) * 2; DOM.body.style.setProperty('--mouse-x', `${x * 10}px`); DOM.body.style.setProperty('--mouse-y', `${y * 10}px`); });
}
...
// ... (setupHeroIndicators and updateHeroIndicators are unchanged and correct)
...
function createCarousel(title, mediaItems) {
    ...
    mediaItems.forEach((item, index) => {
        ...
        // [NEW] Staggered animation delay for the jump-in effect
...
/**
 * [REPLACED] Sets up the "jump and fade-in" animation for carousels.
 */
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Animate posters inside the carousel
                entry.target.querySelectorAll('.movie-poster').forEach(poster => {
                    poster.classList.add('is-scrolled-in');
                });
...
/* --- 6. MODAL LOGIC (REBUILT FROM SCRATCH FOR PERFORMANCE) --- */
async function openDetailsModal(mediaItem, clickedImgElement) {
    ...
    // Function to render the skeleton UI
    ...
    // 1. Start the animation with the skeleton UI immediately
    if (useTransition) {
    ...
    // 2. Fetch data in the background while the animation runs
    ...
    // 3. Once data arrives, populate the real content
    populateModalContent(details, mediaType);
}

function populateModalContent(details, mediaType) {
    // This function replaces the skeleton with the real, fetched content
    const bannerUrl = details.backdrop_path ? `url(https://image.tmdb.org/t/p/original${details.backdrop_path})` : '';
    const finalHtml = buildModalHtml(details, mediaType); // Use the same safe builder
    ...
    const newContent = DOM.modalContent.querySelectorAll('.modal-title-group > *, .modal-info-column > *');
    newContent.forEach(el => el.classList.add('fade-in-content'));

    // Setup interactivity (seasons, AI)
    setupModalInteractivity(details);
}

function buildModalHtml(details, mediaType) {
    // This is the same safe HTML builder function from the last working version
...
function closeModal() {
    // ... (This function is unchanged and correct)
    DOM.body.classList.remove('modal-open'); DOM.modalOverlay.classList.remove('active'); DOM.modal.classList.remove('active'); setTimeout(() => { DOM.modalScrollContainer.scrollTop = 0; DOM.modalContent.innerHTML = ''; }, 600);
}

async function displayAiInsights(title, overview) {
    // ... (This function is unchanged and correct)
    const container = document.getElementById('ai-insights'); if (!container) return; container.innerHTML = `<p class="ai-loading">Asking the AI for a vibe check...</p>`; const insights = await fetchAiVibe(title, overview); if (insights && insights.vibe_check && insights.smart_tags) { container.innerHTML = `<div class="vibe-check"><h3 class="section-title">AI Vibe Check</h3><p>"${insights.vibe_check}"</p></div><div class="smart-tags"><h3 class="section-title">Smart Tags</h3><div class="tags-container">${insights.smart_tags.map(tag => `<span>${tag}</span>`).join('')}</div></div>`; } else { container.innerHTML = ''; }
}

/* --- 7. SEARCH LOGIC & 8. SWIPE LOGIC --- */
// ... (These sections are unchanged and correct)
