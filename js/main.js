...
async function openDetailsModal(mediaItem) {
    ...
    DOM.modal.classList.remove('active'); // Ensure it's ready to transition in

    DOM.body.classList.add('modal-open');
    DOM.modalOverlay.classList.add('active');
    
    // Use a timeout to allow the background overlay to fade in before the modal scales up
    ...
    // **NEW: Build Seasons HTML if it's a TV show**
    const seasonsHtml = (mediaType === 'tv' && details.seasons?.length > 0)
        ? `
        <div class="modal-seasons">
            <h3>Seasons</h3>
            <div class="seasons-list">
                ${details.seasons.map(season => season.poster_path ? `
