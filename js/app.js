// =====================================================
// Personal Cinema - app.js (No Login Version)
// =====================================================
...
(function () {
    ...
    async function syncWatchlist() {
        ...
        } catch (error) { console.error('Could not sync watchlist.', error); }
    }
    ...
    async function initDetailsPage() {
        ...
        try {
            ...
            // Determine the "Watch Now" URL
            let watchUrl = '';
            if (type === 'movie') {
                watchUrl = `${config.watchBaseUrl}/movie/${id}?play=true`;
            } else if (type === 'tv' && details.seasons?.length > 0) {
                // Link to Season 1, Episode 1 by default
                const firstSeason = details.seasons.find(s => s.season_number > 0) || details.seasons[0];
                watchUrl = `${config.watchBaseUrl}/tv/${id}/${firstSeason.season_number}/1?play=true`;
            }

            mainContent.innerHTML = ` <div class="details-backdrop-container"><img src="${getImageUrl(details.backdrop_path, 'original')}" class="details-backdrop" alt=""></div> <div class="details-content-container"> <img src="${getImageUrl(details.poster_path, 'w500')}" class="details-poster" alt="${title} Poster"> <div class="details-info"> <h1 class="title">${title}</h1> <div class="meta-pills"> <div class="meta-pill">${year}</div> ${genres} <div class="meta-pill">${runtime}</div> ${rating} </div> <p class="overview">${details.overview}</p> <div class="action-buttons" id="action-buttons"></div> </div> </div> <div class="container" id="details-lower-section"></div> `;
            ...
            if (watchUrl) {
                ...
                watchButton.target = '_blank'; // Open in new tab
                actionButtonsContainer.appendChild(watchButton);
            }
            ...
            watchlistButton.addEventListener('click', async () => { /* ... existing logic ... */ });
            ...
            aiButton.addEventListener('click', async () => { /* ... existing logic ... */ });
    ...
    async function initWatchlistPage() {
        ...
        try {
            await syncWatchlist();
            const items = Array.from(state.watchlist).map(id => {
                // This is a simplified display. A better way would be to store the whole object in KV.
                // For now, we just show a message.
                // To display full cards, the `update-watchlist` GET would need to return full objects.
            });
        ...
        } catch (error) { grid.innerHTML = '<p>Could not load your watchlist.</p>'; }
    }
    ...
    function initGlobalComponents() {
        ...
        if (!header) return;
        header.innerHTML = ` <nav class="main-nav container"> <a href="/" class="nav-logo">CINEMA</a> <div class="nav-links"> <a href="/" class="${document.body.classList.contains('home-page') ? 'active' : ''}">Home</a> <a href="/watchlist.html" class="${document.body.classList.contains('watchlist-page') ? 'active' : ''}">Watchlist</a> </div> </nav> `;
