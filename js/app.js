// =====================================================
// Personal Cinema - app.js (Definitive Workaround Version)
// =====================================================
...
(function () {
        ...
        try {
            ...
            if (state.currentUser) {
                ...
                aiButton.addEventListener('click', async () => {
                    ...
                    try {
                        const aiData = await apiRequest('get-ai-insights', { movieTitle: title, genres: details.genres.map(g => g.name).join(', ') });
    ...
    // --- 7. SCRIPT EXECUTION (DEFINITIVE WORKAROUND LOGIC) ---
    function runApp() {
        if (window.netlifyIdentity) {
            // Attach event listeners
            window.netlifyIdentity.on('init', user => {
                state.currentUser = user;
                router(); // Run the main app router
            });
            ...
            // *** THE WORKAROUND ***
            // Manually initialize the widget with the full URL.
            window.netlifyIdentity.init({
                APIUrl: "https://pbmoviexp.netlify.app/.netlify/identity"
            });
        } else {
            // Fallback if the widget script fails to load entirely
